...
def published(*versions):
    return lambda *args, **kwargs: {'name': 'jocaagura_domain_core', 'versions': [{'version': v} for v in versions]}
...
class ReleaseDispatchTests(unittest.TestCase):
    ...
    def test_only_master_event_context_and_exact_sha(self):
        env = {'GITHUB_REPOSITORY': 'grupo-jocaagura/jocaagura_domain_core', 'GITHUB_REF': 'refs/heads/master', 'GITHUB_EVENT_NAME': 'push', 'GITHUB_SHA': SHA}
    ...
    def test_actual_publisher_guard_rejects_branch_dispatch_and_unrelated_events(self):
        ...
        for event, ref, sender, valid in [('workflow_dispatch', 'tag', 'Bot', True), ('workflow_dispatch', 'branch', 'User', False), ('push', 'tag', 'User', True), ('push', 'tag', 'Bot', False), ('pull_request', 'tag', 'User', False)]:
            result = subprocess.run([str(bash), '--noprofile', '--norc', '-e'], input=script, text=True, capture_output=True, env=dict(os.environ, GITHUB_REPOSITORY='grupo-jocaagura/jocaagura_domain_core', EVENT_NAME=event, REF_TYPE=ref, SENDER_TYPE=sender))
