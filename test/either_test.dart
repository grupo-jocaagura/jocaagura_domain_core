...
void main() {
  group('Either and FutureEitherExtensions', () {
    const Either<String, int> left = Left<String, int>('failure');
    const Either<String, int> right = Right<String, int>(7);

    test('Given const branches When comparing payloads and generic types Then equality hash and diagnostics agree', () {
    ...
    test('Given nullable payloads When comparing and transforming Then null remains a valid branch value', () {
      ...
      expect(
        nilRight.map<int>((int? i) => i ?? 9),
        const Right<String?, int>(9),
      );
    ...
    for (final Either<String, int> branch in <Either<String, int>>[
      left,
      right,
    ]) {
      test(
        'Given $branch When dispatching with when match or fold Then exactly one callback runs',
        () {
          int calls = 0;
      ...
      test(
        'Given $branch When mapping chaining or applying effects Then only the selected callback runs',
        () {
          int successes = 0;
          int failures = 0;
          expect(
      ...
      test(
        'Given $branch When using synchronous or asynchronous transforms Then async methods retain the branch',
        () async {
          int calls = 0;
      ...
      test(
        'Given future $branch When composing extensions Then the selected branch is retained',
        () async {
    ...
    test('Given throwing synchronous callbacks When each selected operation runs Then the original error propagates', () {
    ...
    test('Given callback or upstream failures When async composition runs Then errors propagate without becoming Left', () async {
    ...
    test('Given typed ErrorItem and Unit When composing through the entrypoint Then the result types are retained', () async {
