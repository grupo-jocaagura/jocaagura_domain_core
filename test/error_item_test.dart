...
void main() {
  group('ErrorItem', () {
    ...
    test('Given const errors When reading fields and enums Then wire keys order and defaults are retained', () {
      expect(
        identical(
          error,
    ...
    test('Given absent null or coerced fields When decoding Then legacy defaults are preserved', () {
    ...
    test('Given every severity and unknown text When decoding Then known values round trip and unknown values use systemInfo', () {
    ...
    test('Given mutable metadata When constructing decoding encoding and copying Then only copyWith owns the outer map', () {
    ...
    test('Given field overrides or null When copying Then replacements apply and null retains values', () {
    ...
    test('Given equal or different metadata When comparing errors Then shallow equality and order-independent hashing agree', () {
      expect(error, error);
      ...
      final ErrorItem a = error.copyWith(
        meta: <String, dynamic>{'a': 1, 'b': 2},
      );
      final ErrorItem b = error.copyWith(
        meta: <String, dynamic>{'b': 2, 'a': 1},
      );
      expect(a, b);
    ...
    test('Given canonical and non-JSON metadata When encoding Then the fixture matches and unsupported values fail', () {
